...
def infer_query_manifest(root: Path, cfg: dict) -> Path:
    output_root = root_join(root, cfg["dataset"]["output_root"])
    candidates = [
...
def infer_output_dirs(root: Path, cfg: dict) -> tuple[Path, Path, Path]:
    output_root = root_join(root, cfg["dataset"]["output_root"])
    return (
        output_root / "descriptors",
        output_root / "retrieval/s8_11d",
        output_root / "reports/s8_11d",
    )


def infer_tile_indexes(root: Path, cfg: dict) -> dict[str, Path]:
    variants = cfg.get("map", {}).get("tile_variants", {})
    out = {}
...
def infer_oracle_csvs(root: Path, cfg: dict, variant_names: list[str]) -> dict[str, Path]:
    output_root = root_join(root, cfg["dataset"]["output_root"])
    out = {}
    for name in variant_names:
        p = output_root / f"metadata/s8_10b_uav_tile_oracle_{name}.csv"
        if not p.exists():
            raise FileNotFoundError(f"Missing oracle CSV for {name}: {p}")
        out[name] = p
    return out
...
def build_variant_paths(
    ...
    cfg: dict,
    tag: str,
    map_cache_root: str | None = None,
) -> dict[str, dict[str, Path]]:
    tile_indexes = infer_tile_indexes(root, cfg)
    oracles = infer_oracle_csvs(root, cfg, list(tile_indexes.keys()))

    if map_cache_root:
        map_desc_dir = root_join(root, map_cache_root)
    else:
        # Default: map cache is in this dataset's descriptor folder.
        # For 45°, pass --map-cache-root outputs/villoc/90_deg/descriptors
        map_desc_dir = root_join(root, cfg["dataset"]["output_root"]) / "descriptors"

    variants = {}
    for name, tile_index in tile_indexes.items():
        variants[name] = {
            "tile_index": tile_index,
            "oracle": oracles[name],
            "map_cache": map_desc_dir / f"s8_11b_dinov2_map_{name}_{tag}.npz",
        }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description= 'S8.11D — Independent DINOv2 retrieval evaluation'
    )
    ...
    parser.add_argument("--device-tag", choices=["cpu", "auto", "cuda"], default="auto")
    parser.add_argument("--map-cache-root", default=None, help="Optional map descriptor directory, e.g. outputs/villoc/90_deg/descriptors",)

    args = parser.parse_args()
    TAG = f"dinov2_vits14_img{args.image_size}_{args.crop_mode}_{args.pooling}_{args.device_tag}"

    cfg = load_yaml(root_join(ROOT, args.config))

    QUERY_CSV = infer_query_manifest(ROOT, cfg)
    QUERY_DESC_DIR, OUT_DIR, REPORT_DIR = infer_output_dirs(ROOT, cfg)

    QUERY_CACHE = QUERY_DESC_DIR / f"s8_11c_dinov2_queries_v_1fps_{TAG}.npz"

    VARIANTS = build_variant_paths(
        root=ROOT,
        cfg=cfg,
        tag=TAG,
        map_cache_root=args.map_cache_root,
    )

...
python scripts/satloc/s6a/s6a_4a_klt_vs_orb_relative_comparison.py \
  --sequence traj01 \
  --resize-long 960 \
  --max-corners 1200 \
  --quality-level 0.01 \
...
def main() -> None:
    ...
    if bool(failed_affine.any()):
        ...
        failure_report = {
            "stage": "S6A.4A_failure_diagnostic",
            "sequence": args.sequence,
            "frames_processed": int(len(manifest)),
            "pairs_processed": int(len(pair_df)),
            "failed_affine_pairs": int(len(failed_df)),
            "failure_rate": float(
                len(failed_df) / max(len(pair_df), 1)
            ),
            "status_counts": {
                str(key): int(value)
            ...
            "important_interpretation": (
                "The fixed-parameter KLT baseline cannot form a "
                "complete trajectory chain when any affine pair "
                "fails. This is a method result, not a script crash. "
                "Inspect whether failures coincide with sharp turns, "
                "large displacement, low overlap, or weak tracks "
                "before adding retries or ORB fallback."
            ),
        }
        ...
        print(
            "\nS6A.4A KLT full-sequence diagnostic stopped "
            "before trajectory accumulation"
        )
        print(
            "--------------------------------------------------"
        )
        print(
            f"Pairs processed:              {len(pair_df)}"
        )
        print(
            f"Failed affine pairs:          {len(failed_df)}"
        )
        print(
            "Failure rate:                "
            f"{100.0 * len(failed_df) / max(len(pair_df), 1):.3f}%"
        )
        print(
            f"Failure statuses:            {status_counts}"
        )
        print("\nFailed pair summary:")
        columns_to_show = [
            "frame_index_a",
        ...
        print(
            failed_df[columns_to_show].to_string(
                index=False
            )
        )
        print("\nSaved diagnostics:")
        ...
        print(
            "\nNo KLT trajectory metrics were produced because "
            "a dead-reckoning chain cannot silently cross a failed "
            "pair. The next decision is either targeted KLT retry "
            "or ORB fallback at only those failed frames."
        )
        return
    ...
    pair_df.to_csv(pair_path, index=False)
    aligned_df.to_csv(trajectory_path, index=False)

...
def find_ranked_csv(
    ...
    run_id = f"{sequence}_token{query_token0_id:04d}_subset_r{candidate_radius}"
    csv_path = Path(
        "outputs/satloc/metadata/s4a_orb_tile_retrieval/subset_debug"
    ) / f"{run_id}_{variant}_ranked_tiles.csv"

    if not csv_path.exists():
        raise FileNotFoundError(
            f"Missing ranked CSV: {csv_path}\n"
            "Run S4A.2 first with the same sequence/query/radius/variant."
        )
...
def select_evidence_rows(ranked_df: pd.DataFrame, top_n: int) -> pd.DataFrame:
    ...
    if len(correct_df) > 0:
        ...
        best_correct = correct_df.sort_values(
            ["score", "ransac_inliers", "good_matches"],
...
def main() -> None:
    ...
    if len(selected_df) == 0:
        raise RuntimeError("No selected rows found for evidence diagnostics.")
    ...
    run_id = (
        ...
        f"_subset_r{args.candidate_radius}_{args.variant}"
    )

    out_meta_dir = Path("outputs/satloc/metadata/s4a_orb_tile_retrieval/evidence_debug") / run_id
    out_report_dir = Path("outputs/satloc/reports/s4a_orb_tile_retrieval/evidence_debug") / run_id
    out_fig_dir = Path("outputs/satloc/figures/s4a_orb_tile_retrieval/evidence_debug") / run_id
    ...
    print(f"Selected tiles: {len(selected_df)}")
    ...
    for _, row in selected_df.iterrows():
        ...
        good_path = out_fig_dir / f"{label}_rank{rank:03d}_tile{tile_index:05d}_good_matches.png"
        inlier_path = out_fig_dir / f"{label}_rank{rank:03d}_tile{tile_index:05d}_ransac_inliers.png"
        ...
        print(
            f"{label:14s} | rank={rank:3d} tile={tile_index:5d} "
            f"containsGT={bool(row['contains_gt'])} "
            f"score={result.score:5.2f} good={result.good_matches:3d} "
            f"inliers={result.ransac_inliers:3d} "
            f"ratio={result.inlier_ratio:.3f} H={result.homography_success}"
        )
    ...
    save_selected_tiles_panel(
        ...
        title=f"S4A.2B selected retrieval evidence | {run_id}",
    )
    ...
    evidence_csv = out_meta_dir / "evidence_tiles.csv"
    evidence_json = out_report_dir / "evidence_summary.json"
